...
**Panel « Dernier film »** (dès qu'un film a été accepté) : affiche/titre/synopsis du dernier film accepté, et sous le synopsis un où-regarder compact (icônes seules, limité + « +N » si la place manque — même endpoint que Résultat/Historique, CIN-103).
...
Carte film : affiche TMDB, titre + année, durée + genres, résumé tronqué (« Lire plus »), où-regarder (3 colonnes Gratuit/Abonnement/Location-achat, voir [Specs API](./api.md) `GET /films/{tmdb_id}/watch-providers`, CIN-103), boutons « Ce soir ✓ » / « Passer ». Overlay swipe LIKE (vert/droite) / SKIP (rouge/gauche).
...
Tap sur une carte → panel de détail (slide-in/modale) qui réaffiche toutes les infos de la fiche film, y compris où-regarder (dispo streaming FR, mêmes 3 colonnes que Résultat — voir [Specs API](./api.md) `GET /films/{tmdb_id}/watch-providers`, CIN-103) — toujours refetché à ce moment, jamais stocké. **La note TMDB (`vote_average`) n'existe plus dans le schéma** : elle n'était affichée nulle part sur Résultat et aurait de toute façon été refetchée ici — inutile de la stocker. Voir Specs DB pour le raisonnement complet.
